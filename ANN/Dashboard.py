...
# Drop Undesirable Columns (Fully Removed)
drop_columns = ['customer_id', 'order_id', 'session_id', 'device_type']  # Add any other unwanted columns
...
def preprocess_data(df):
    categorical_columns = df.select_dtypes(include=['object']).columns  # Auto-detect categorical columns
...
fig, ax = plt.subplots(figsize=(8, 5))
sns.barplot(x="loyalty_program", y="churned", data=df, palette="coolwarm", ax=ax)
ax.set_title("Loyalty Program Impact on Churn")
st.pyplot(fig)

# Social Media Engagement vs Churn
fig, ax = plt.subplots(figsize=(8, 5))
sns.boxplot(x="churned", y="social_media_engagement", data=df, palette="coolwarm", ax=ax)
ax.set_title("Social Media Engagement vs Churn")
st.pyplot(fig)
